...
val_dataloader = torch.utils.data.DataLoader(val_dataset, batch_size=4, shuffle=True, num_workers=2,
                                                 pin_memory=False)
...
while steps < 64000 and epoch < 100:  # for epoch in range(num_epochs):
    print('Step {}/{}'.format(steps, 64000))
    print('-' * 10)

    epoch += 1
    
    training_loc_loss_list = []
    training_cls_loss_list = []
    training_tot_loss_list = []
    training_acc_list = []
    ...
    for phase in ['train', 'test']:
        ...
        for data in dataloaders[phase]:
            ...
            if num_iter == num_steps_per_update and phase == 'train':
                ...
                if steps % 10 == 0:
                    ...
                    training_loc_loss_list.append(train_loc_loss)
                    training_cls_loss_list.append(train_cls_loss)
                    training_tot_loss_list.append(train_tot_loss)
        ...
        if phase == 'test':
            ...
            if val_score > best_val_score or epoch % 2 == 0:
                ...
                print(model_name)
            epoch_tot_loss = (tot_loss * num_steps_per_update) / num_iter
            epoch_loc_loss = tot_loc_loss / num_iter
            epoch_cls_loss = tot_cls_loss / num_iter
            ...
            performance_metrics['val_loc_loss'].append(epoch_loc_loss)
            performance_metrics['val_cls_loss'].append(epoch_cls_loss)
            performance_metrics['val_tot_loss'].append(epoch_tot_loss)
            performance_metrics['val_acc'].append(epoch_acc)
            performance_metrics['train_loc_loss'].append(training_loc_loss_list)
            performance_metrics['train_cls_loss'].append(training_cls_loss_list)
            performance_metrics['train_tot_loss'].append(training_tot_loss_list)
            performance_metrics['train_acc'].append(training_acc_list)
...
train_cls_loss_mean = [np.mean(epoch_data) for epoch_data in train_cls_loss]
train_acc = performance_data['train_acc']
train_acc_mean = [np.mean(epoch_data) for epoch_data in train_acc]
...
ax1.plot(epochs, train_cls_loss_mean, 'bo-', label='Training Classification Loss')
...
# Plot for accuracy
ax2.plot(epochs, train_acc_mean, 'ko-', label='Training Accuracy')
...
# Display the plots
plt.savefig('retrain_plot.png', dpi=300)
plt.close(fig)
